#!/usr/bin/env python3
"""
harvest_news.py — the news wire: who owns the newsroom, what replaced the
reporting, who is paying to pollute it, and what happens to the people still
doing the work.

After "The News Industry". Ownership concentration — the six companies holding
most of one national market, and the two platforms holding the global one. The
shift the section quotes from public-service orientation to a profit-first
model, and what follows: hard news displaced by stories with no policy
connection, newsroom jobs falling since 1990, and the gap filled with cheap,
aggregated, engagement-optimised filler. Then machine-made news, and the
content-farm count the section tracks across sixteen languages. Then who is
paying to pollute it — state cyber troops, corporate campaigns in tobacco, drug
and oil, and the covert anti-vax operation it names. Then the framing of
opponents as enemies. And finally what happens to the journalists: press
freedom in a minority of countries, surveillance, censorship, lawsuits,
jailings, and a journalist murdered on average every three and a half days.

This is a feed ABOUT the news industry, not a news feed. Ordinary reporting is
refused — every subject term carries the industry words it must appear beside.
...
# --------------------------------------------------------------------------
USER_AGENT = ("Mozilla/5.0 (compatible; news-feed/1.0; "
...
# THE SUBJECTS
#
# Who owns the news, what replaced the reporting, who is paying to pollute
# it, and what happens to the people still doing the work.
#
# This is a feed ABOUT the news industry, not a news feed. Every subject term
# carries the industry words it must appear beside, so ordinary reporting —
# the overwhelming majority of what any wire returns — cannot reach it.
# --------------------------------------------------------------------------
TOPICS = [
    ("ownership", "Who owns the news", [
        ("media ownership", []), ("media concentration", []), ("media consolidat*", []),
        ("newspaper group", ["acquir*", "sold", "merger", "owner", "stake"]),
        ("broadcaster", ["acquir*", "sold", "merger", "owner", "stake", "privatis*", "privatiz*"]),
        ("media group", ["acquir*", "sold", "merger", "owner", "stake", "control"]),
        ("proprietor", []), ("media mogul", []), ("oligarch", ["media", "newspaper", "channel", "outlet"]),
        ("takeover", ["newspaper", "broadcaster", "media", "publisher", "channel"]),
        ("editorial independence", []), ("editorial control", ["owner", "interference", "guarantee"]),
        ("cross-ownership", []), ("plurality", ["media", "review", "test", "concern"]),
    ]),
    ("profit", "Shareholder value over public service", [
        ("shareholder value", ["media", "news", "publisher", "broadcaster"]),
        ("private equity", ["newspaper", "media", "publisher", "local news", "broadcaster"]),
        ("hedge fund", ["newspaper", "media", "publisher", "local news"]),
        ("cost-cutting", ["newsroom", "newspaper", "broadcaster", "media"]),
        ("asset stripping", ["media", "newspaper", "publisher"]),
        ("public service", ["broadcast*", "remit", "obligation", "media", "mandate"]),
        ("profit-first", []), ("commercial pressure", ["newsroom", "editorial", "coverage"]),
        ("advertiser", ["pressure", "pulled", "influence", "coverage", "boycott"]),
        ("sponsored", ["coverage", "editorial", "content", "undisclosed", "section"]),
    ]),
    ("softnews", "What replaced the reporting", [
        ("hard news", []), ("soft news", []), ("infotainment", []),
        ("clickbait", []), ("engagement-optimis*", []), ("engagement-optimiz*", []),
        ("churnalism", []), ("press release", ["republished", "verbatim", "churn", "rewritten"]),
        ("aggregat*", ["content", "news", "site", "rewritten", "lifted"]),
        ("rewritten", ["without credit", "other outlets", "lifted", "reporting"]),
        ("policy coverage", ["decline", "fell", "share", "less"]),
        ("political coverage", ["horse race", "declin*", "substance", "criticis*"]),
    ]),
    ("jobs", "Newsrooms and news deserts", [
        ("newsroom", ["cuts", "layoff*", "redundanc*", "closure", "shrunk", "jobs"]),
        ("layoff*", ["journalist*", "newsroom", "media", "publisher", "broadcaster"]),
        ("redundanc*", ["journalist*", "newsroom", "media", "publisher"]),
        ("news desert", []), ("local news", ["closure", "decline", "cuts", "desert", "crisis", "funding"]),
        ("local newspaper", ["closed", "closure", "merged", "cuts"]),
        ("journalism jobs", []), ("staff cuts", ["newsroom", "media", "broadcaster", "publisher"]),
        ("title closed", []), ("print edition", ["ends", "closing", "final", "ceases"]),
        ("freelance", ["rates", "journalist*", "precarious", "unpaid"]),
    ]),
    ("contentfarms", "Machine-made news", [
        ("content farm", []), ("ai-generated", ["news", "article", "site", "content", "outlet", "undisclosed"]),
        ("ai slop", []), ("newsguard", []), ("unreliable ai", []),
        ("synthetic", ["article", "news", "byline", "presenter", "anchor"]),
        ("fake byline", []), ("ai byline", []), ("undisclosed ai", []),
        ("repackag*", ["content", "reporting", "articles", "old"]),
        ("scraped", ["reporting", "articles", "content", "without permission"]),
        ("pink slime", []), ("automated news", []),
    ]),
    ("statedisinfo", "State information operations", [
        ("cyber troop*", []), ("troll farm", []), ("bot network", []),
        ("influence operation", []), ("information operation", []),
        ("psychological operation", []), ("psyop", []),
        ("coordinated inauthentic", []), ("state-backed", ["campaign", "media", "accounts", "network", "disinformation"]),
        ("state media", ["campaign", "propaganda", "expansion", "funding", "ban"]),
        ("attributed", ["campaign", "operation", "network", "state", "accounts"]),
        ("takedown", ["network", "accounts", "operation", "coordinated"]),
        ("foreign propaganda", []), ("covert campaign", []),
    ]),
    ("corpdisinfo", "Bought and industrial disinformation", [
        ("disinformation", ["campaign", "industry", "paid", "commercial", "funded", "network"]),
        ("misinformation", ["campaign", "study", "spread", "research", "policy", "platform"]),
        ("front group", []), ("astroturf*", []),
        ("manufactured doubt", []), ("merchants of doubt", []),
        ("industry-funded", ["campaign", "research", "study", "group", "media"]),
        ("public relations firm", ["hired", "campaign", "contract", "leaked"]),
        ("dark pr", []), ("reputation management", []),
        ("paid coverage", []), ("brown envelope", []),
    ]),
    ("framing", "Framing and enemy-making", [
        ("enemy of the people", []), ("enemy of the state", ["press", "journalist*", "media"]),
        ("framing", ["coverage", "media", "protest*", "migrant*", "study", "analysis"]),
        ("dehumanis*", ["coverage", "media", "language", "migrant*"]),
        ("moral panic", []), ("scapegoat*", ["media", "coverage", "migrant*", "minorit*"]),
        ("coverage of migrants", []), ("media portrayal", []),
        ("hate speech", ["broadcast*", "outlet", "media", "regulator", "licence"]),
        ("incitement", ["broadcast*", "media", "outlet", "programme"]),
        ("smear campaign", ["journalist*", "outlet", "media"]),
    ]),
    ("bias", "Rating the outlets", [
        ("media bias", []), ("bias rating", []), ("accuracy rating", []),
        ("trust in media", []), ("trust in news", []), ("news avoidance", []),
        ("credibility", ["rating", "outlet", "assessment", "score", "study"]),
        ("digital news report", []), ("reuters institute", []),
        ("audience trust", []), ("perceived bias", []),
    ]),
    ("verification", "Checking, correcting and being wrong", [
        ("fact-check*", []), ("fact check", []),
        ("correction", ["issued", "published", "policy", "failed", "buried"]),
        ("retraction", ["article", "story", "outlet", "published"]),
        ("debunk*", []), ("prebunk*", []), ("media literacy", []),
        ("verification", ["osint", "footage", "claims", "images", "unit"]),
        ("deepfake", ["news", "verification", "detected", "broadcast", "presenter"]),
        ("provenance", ["content", "image", "c2pa", "watermark"]),
    ]),
    ("pressfreedom", "Press freedom overall", [
        ("press freedom", []), ("media freedom", []),
        ("world press freedom index", []), ("reporters without borders", []),
        ("committee to protect journalists", []), ("cpj", []),
        ("freedom of the press", []), ("press freedom ranking", []),
        ("media law", ["restrictive", "passed", "criticis*", "chilling"]),
        ("foreign agent", ["media", "outlet", "journalist*", "law", "designated"]),
        ("accreditation", ["revoked", "withdrawn", "denied", "journalist*", "press"]),
    ]),
    ("jailings", "Detention, prosecution and exile", [
        ("journalist*", ["jailed", "detained", "arrested", "sentenced", "charged", "convicted",
                          "imprisoned", "released", "exile", "deported"]),
        ("reporter", ["jailed", "detained", "arrested", "sentenced", "charged", "convicted"]),
        ("editor", ["jailed", "detained", "arrested", "sentenced", "charged", "convicted"]),
        ("press card", ["revoked", "withdrawn", "seized"]),
        ("raid", ["newsroom", "outlet", "journalist*", "broadcaster", "offices"]),
        ("espionage charge", ["journalist*", "reporter", "media"]),
        ("terrorism charge", ["journalist*", "reporter", "media"]),
        ("in exile", ["journalist*", "outlet", "newsroom", "editor"]),
    ]),
    ("killings", "Journalists killed", [
        ("journalist*", ["killed", "murdered", "shot", "assassinated", "died covering"]),
        ("reporter", ["killed", "murdered", "shot", "assassinated"]),
        ("media worker", ["killed", "murdered", "wounded"]),
        ("impunity", ["killing", "murder", "journalist*", "index"]),
        ("unsolved", ["murder", "killing", "journalist*"]),
        ("press casualt*", []), ("safety of journalists", []),
    ]),
    ("surveillance", "Watching the reporters", [
        ("pegasus", []), ("spyware", ["journalist*", "reporter", "phone", "targeted", "forensic"]),
        ("surveillance", ["journalist*", "reporter", "newsroom", "source", "targeted"]),
        ("phone seiz*", ["journalist*", "reporter"]),
        ("source protection", []), ("protect sources", []), ("shield law", []),
        ("metadata", ["journalist*", "accessed", "records", "seized"]),
        ("data request", ["journalist*", "records", "telecom", "police"]),
        ("infiltrat*", ["newsroom", "journalist*", "undercover", "police"]),
    ]),
    ("censorship", "Blocking and shutting down", [
        ("censorship", ["media", "press", "outlet", "broadcast*", "online", "state"]),
        ("blocked", ["website", "outlet", "news site", "broadcast*", "access"]),
        ("internet shutdown", []), ("licence revoked", ["broadcast*", "channel", "station", "outlet"]),
        ("license revoked", ["broadcast*", "channel", "station", "outlet"]),
        ("banned", ["outlet", "broadcast*", "newspaper", "channel", "journalist*"]),
        ("takedown request", []), ("content removal", ["order", "demand", "government", "request"]),
        ("prior restraint", []), ("injunction", ["publication", "reporting", "story", "gag"]),
        ("gag order", []),
    ]),
    ("legal", "Lawsuits as a method", [
        ("slapp", []), ("strategic lawsuit", []),
        ("defamation", ["suit", "claim", "case", "journalist*", "outlet", "ruling", "chilling"]),
        ("libel", ["suit", "case", "ruling", "tourism", "chilling"]),
        ("anti-slapp", []), ("legal threat", ["journalist*", "outlet", "reporting", "chilling"]),
        ("damages", ["defamation", "libel", "outlet", "journalist*"]),
        ("settled", ["defamation", "libel", "lawsuit", "outlet", "broadcaster"]),
        ("subpoena", ["journalist*", "records", "notes", "source"]),
    ]),
    ("platforms", "Distribution and dependency", [
        ("referral traffic", []), ("news traffic", ["fell", "collapse", "decline", "search", "platform"]),
        ("algorithm change", ["publisher*", "traffic", "news", "reach"]),
        ("news tab", []), ("news ban", ["platform", "facebook", "meta", "google"]),
        ("bargaining code", []), ("link tax", []), ("neighbouring right*", []),
        ("licensing deal", ["publisher*", "news", "ai", "content"]),
        ("scraping", ["publisher*", "news", "ai", "content", "lawsuit"]),
        ("ai overview", []), ("zero-click", []),
    ]),
    ("funding", "Who pays for it", [
        ("paywall", []), ("subscription", ["news", "publisher", "outlet", "growth", "decline"]),
        ("public broadcaster", ["funding", "cut", "licence fee", "independence", "board"]),
        ("licence fee", []), ("license fee", ["broadcast*", "frozen", "abolish*"]),
        ("state funding", ["media", "outlet", "advertising", "allocation"]),
        ("government advertising", ["allocation", "withheld", "favour*", "outlet"]),
        ("philanthropic funding", ["journalism", "news", "outlet"]),
        ("nonprofit newsroom", []), ("media fund", []), ("journalism fund", []),
    ]),
    ("cost", "What it costs everyone else", [
        ("cost of disinformation", []), ("economic cost", ["disinformation", "misinformation"]),
        ("spread faster", ["false", "true", "news", "rumour"]),
        ("study finds", ["misinformation", "disinformation", "false news", "spread"]),
        ("election", ["disinformation", "misinformation", "false claims", "integrity of information"]),
        ("public health", ["misinformation", "disinformation", "anti-vax", "campaign"]),
        ("anti-vax", ["campaign", "covert", "military", "pentagon", "operation"]),
    ]),
    ("resistance", "What is set against it", [
        ("investigative journalism", ["funding", "award", "network", "collaboration", "unit"]),
        ("cross-border", ["investigation", "collaboration", "consortium", "journalists"]),
        ("consortium", ["journalists", "investigation", "leak", "reporting"]),
        ("whistleblower", ["protection", "journalist*", "disclosure", "law"]),
        ("press council", []), ("media regulator", ["ruling", "complaint", "upheld", "code"]),
        ("union", ["journalist*", "newsroom", "recognition", "strike", "collective"]),
        ("worker-owned", ["outlet", "newsroom", "cooperative"]),
        ("cooperative", ["newsroom", "outlet", "media", "reader-owned"]),
        ("campaign", ["press freedom", "media freedom", "journalist*", "anti-slapp"]),
    ]),
]

ANCHOR = [
    # News-industry language. Ordinary reporting is not this subject; who owns
    # the newsroom, what replaced the reporting, who is paying to pollute it,
    # and what happens to the people still doing the work, is.
    "media ownership", "media concentration", "editorial independence",
    "newsroom", "news desert", "local news", "journalism", "journalist",
    "press freedom", "media freedom", "reporters without borders",
    "committee to protect journalists", "world press freedom index",
    "content farm", "ai-generated news", "newsguard", "churnalism", "clickbait",
    "disinformation", "misinformation", "influence operation", "cyber troops",
    "troll farm", "coordinated inauthentic", "fact-check", "media literacy",
    "media bias", "trust in news", "news avoidance", "digital news report",
    "slapp", "anti-slapp", "defamation suit", "shield law", "source protection",
    "press accreditation", "internet shutdown", "licence revoked",
    "spyware", "pegasus", "surveillance of journalists",
    "paywall", "public broadcaster", "licence fee", "bargaining code",
    "referral traffic", "link tax", "neighbouring rights",
    "press council", "media regulator", "investigative journalism",
    "enemy of the people", "state media", "propaganda",
]

BLOCK = [
    # This is a feed about the news industry, so ordinary news must stay out.
    # The subject gate does most of that work; these catch the trade puff and
    # the listings that would otherwise slip through on industry vocabulary.
    "ratings win", "viewing figures", "audience share record", "tv listings",
    "watch live", "live blog", "as it happened", "in pictures", "video shows",
    "presenter announced", "new host", "joins the show", "signs new deal",
    "podcast launch", "channel launch", "show returns", "series finale",
    "royal television society", "bafta", "emmy", "press awards shortlist",
    "columnist writes", "opinion piece argues", "letters to the editor",
    ...
    "transfer rumour", "match report", "quarterly results", "share price",
]
...
DECIDED = [
    ...
    "fined", "settled", "convicted", "acquitted", "sentenced", "released",
    "ordered", "blocked", "licence revoked", "complaint upheld", "takedown ordered",
]
INSTITUTIONAL = [
    "reporters without borders", "committee to protect journalists", "unesco",
    "council of europe", "european commission", "european parliament",
    "ohchr", "special rapporteur", "european court of human rights",
    "media regulator", "press council", "ofcom", "information commissioner",
    "reuters institute", "pew research", "newsguard", "freedom house",
    "auditor general", "national audit office", "parliamentary committee",
    "select committee", "official gazette", "court filing",
    "peer-reviewed", "published in", "study finds", "working paper", "dataset",
    "official data", "government figures", "national statistics",
]
MEASURED = [
    "per cent", "percent", "%", "million", "billion", "one in", "1 in",
    "figures show", "fell by", "rose by", "increase of", "decrease of",
    "estimated", "median", "average", "ranked", "index", "share of", "rate of",
    "number of journalists", "titles closed", "jobs lost", "sites identified",
]
PENDING = [
...
def run(dry_run=False, fixtures=None):
    ...
    payload = {
        ...
        "languages": languages,
        "standings": [
            {"id": "official", "label": "Regulators, courts & UN bodies"},
            {"id": "research", "label": "Research & media monitoring"},
            {"id": "press", "label": "Press"},
            {"id": "rights", "label": "Press freedom & journalist organisations"},
        ],
    ...
    if dry_run:
        print("\n--dry-run: wire_news.json not written")
        return payload
